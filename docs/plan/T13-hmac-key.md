# T13 — `HMAC_KEY` env; drop `app_secrets`
...
repository root with `-- +goose Up` sections, `make migrate`, the same
command in CI, `storetest/goose.go` applying it in tests, and
`storetest/migrate_test.go` asserting `Store.Open` runs no SQL.
