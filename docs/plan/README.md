...
|---|---|---|---|---|---|
| [T01](T01-evals-go.md) | Rewrite evals in Go | todo | M | — | §4 |
...
| Now | Target |
|---|---|
| Durable state in `DATA_DIR` (TOML users, `tokens.json`, SQLite cache, `oauth/hmac.key`, in-memory codes) | PostgreSQL only (`DATABASE_URL`) |
| `WRITE_MODE` / `WRITE_ALLOW` / budgets / `CORP_SCOPES` / `COMPAT_DATE` env | Constants (SPEC §2); all tools always registered |
