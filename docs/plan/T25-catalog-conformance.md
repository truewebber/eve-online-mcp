# T24 — Catalog conformance and the server instructions
...
proof, and it is what T27's linter later checks from the outside.

## Why this is one Composer session

It is one file of contract read against one package of code, with no
design decisions left in it. T23 did the behaviour; this is the wording,
...
- Leave the instructions talking about "which characters are authorized"
  or an allowance without an error budget. T13 and T19 changed both.
...
`patchBounds` is the single place numeric bounds are applied; T25 adds
...
- [ ] `go test ./...` and `make lint` pass, and `go run ./evals lint` is
      clean
