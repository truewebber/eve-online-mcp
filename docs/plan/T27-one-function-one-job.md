# T26 — One function, one job; one result
...
It lands here because the tool surface stops moving at T25. Doing it
earlier means splitting functions that T23 and T25 were about to
rewrite.
...
| Site | Returns |
|---|---|
| `usecase/eve/common.go` `page` | `([]map[string]any, map[string]any)` — T25 replaces it; verify |
...
| `evals/main.go` `lintProps` | two string slices |
| `usecase/session/persist.go` `GetConfirm` | three values — T14 fixes it; verify |
...
`usecase/oauth` and `usecase/eve/writes.go` were split by T16 and T23;
