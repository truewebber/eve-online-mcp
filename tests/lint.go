...
func inspectTool(tool map[string]any) findings {
	...
	propsOut := lintProps(name, props)
	out.failures = append(out.failures, propsOut.failures...)
	out.warnings = append(out.warnings, propsOut.warnings...)

	return out
}

func lintProps(name string, props map[string]any) findings {
	var out findings
	for param, specAny := range props {
		spec := j.Map(specAny)
		if j.Str(spec[fieldDescription]) == "" {
			out.failures = append(out.failures, name+"."+param+": no description in the schema")
		}
		...
		if j.Str(spec[fieldType]) == typeInteger {
			if _, ok := spec["maximum"]; !ok && !strings.HasSuffix(param, "_id") {
				out.warnings = append(out.warnings, name+"."+param+": unbounded integer, no maximum in schema")
			}
		}
		switch param {
		case "character":
			out.failures = append(out.failures, name+"."+param+": a connection is one character; tools do not take a character parameter")
		case "user", "id", "target_id", "data", "input":
			out.warnings = append(out.warnings, name+"."+param+": ambiguous parameter name")
		}
	}
	if _, hasLimit := props[fieldLimit]; hasLimit {
		_, hasFormat := props["response_format"]
		if !hasFormat && needsResponseFormat(name) {
			out.warnings = append(out.warnings, name+": has `limit` but no `response_format`")
		}
	}
