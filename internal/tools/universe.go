...
func registerUniverse(s *mcp.Server, a *app.App) {
	...
	addTool(s, &mcp.Tool{
		...
		return Call(ctx, func(a *app.App) (any, error) {
			...
			ch := make(chan box, 3)
			go func() { r, err := a.ESI.Get(fmt.Sprintf("/universe/systems/%d", sid), nil, nil, nil); ch <- box{r, err} }()
...
func searchWithFallback(a *app.App, characterID int, categories []string, query string, strict bool) (map[string][]int, string, error) {
