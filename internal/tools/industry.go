...
func registerIndustry(s *mcp.Server, a *app.App) {
	type jobsIn struct {
...
func industryJobsResult(a *app.App, character string, cid int, data any, stale string, limit int, conciseMode, withInstaller bool) (map[string]any, error) {
	jobs := j.Maps(data)
	if len(jobs) == 0 {
		return map[string]any{
