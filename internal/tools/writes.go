...
func registerWrites(s *mcp.Server, a *app.App) {
	if a.Settings.CapabilityEnabled("waypoint") {
		registerWaypoint(s, a)
	}
	if a.Settings.CapabilityEnabled("openwindow") {
		registerOpenWindow(s, a)
	}
	if a.Settings.CapabilityEnabled("fittings") {
		registerWriteFittings(s, a)
	}
	if a.Settings.CapabilityEnabled("mail_organize") {
		registerMailOrganize(s, a)
	}
	if a.Settings.CapabilityEnabled("mail_send") {
		registerMailSend(s, a)
	}
	if a.Settings.CapabilityEnabled("contacts") {
		registerContacts(s, a)
	}
	if a.Settings.CapabilityEnabled("calendar") {
		registerCalendar(s, a)
	}
}

func registerWaypoint(s *mcp.Server, a *app.App) {
	type in struct {
...
func registerOpenWindow(s *mcp.Server, a *app.App) {
	type in struct {
...
func registerWriteFittings(s *mcp.Server, a *app.App) {
	type saveIn struct {
...
func registerMailOrganize(s *mcp.Server, a *app.App) {
	type markIn struct {
...
func registerMailSend(s *mcp.Server, a *app.App) {
	type in struct {
...
func registerContacts(s *mcp.Server, a *app.App) {
	type setIn struct {
...
func registerCalendar(s *mcp.Server, a *app.App) {
	type in struct {
...
func resolveContacts(a *app.App, namesIn []string) ([]names.NameMatch, map[string]any, error) {
	only := []string{"characters", "corporations", "alliances"}
...
func resolveDestination(a *app.App, name string, characterID int) (map[string]any, error) {
...
func resolveEntity(a *app.App, name string, characterID int, kind string) (map[string]any, error) {
