...
func registerCharacter(s *mcp.Server, a *app.App) {
	...
	addTool(s, &mcp.Tool{
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in skillsIn) (*mcp.CallToolResult, any, error) {
		return Call(ctx, func(a *app.App) (any, error) {
			...
			trainedOnly := boolDef(in.TrainedOnly, true)
			...
			sort.Slice(rows, func(i, k int) bool { return j.Str(rows[i]["skill"]) < j.Str(rows[k]["skill"]) })
			visible, meta := page(rows, limitOr(in.Limit, 20), "Narrow with `search`, or raise `limit`.")
			at5 := 0
			for _, s := range skills {
			...
			out := merge(map[string]any{
				...
				"at_level_5": at5, "matching": len(rows), "data_age": result.StaleNote(),
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in queueIn) (*mcp.CallToolResult, any, error) {
		return Call(ctx, func(a *app.App) (any, error) {
			token, err := a.ResolveCharacter(in.Character)
			if err != nil {
				return nil, err
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in clonesIn) (*mcp.CallToolResult, any, error) {
		return Call(ctx, func(a *app.App) (any, error) {
			token, err := a.ResolveCharacter(in.Character)
			if err != nil {
				return nil, err
			...
			return map[string]any{
				...
				"active_implants": active, "jump_clones": jumps,
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in standingsIn) (*mcp.CallToolResult, any, error) {
		return Call(ctx, func(a *app.App) (any, error) {
			token, err := a.ResolveCharacter(in.Character)
			if err != nil {
				return nil, err
			...
			sort.Slice(lpData, func(i, k int) bool { return j.Float(lpData[i]["loyalty_points"]) > j.Float(lpData[k]["loyalty_points"]) })
			...
			out := merge(map[string]any{
				"character": token.CharacterName, "loyalty_points": lpRows, "standings": visible,
			}, meta)
			if standingsErr != nil {
