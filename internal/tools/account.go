...
func registerAccount(s *mcp.Server, a *app.App) {
	addTool(s, &mcp.Tool{
	...
	addTool(s, &mcp.Tool{
		...
		return Call(ctx, func(a *app.App) (any, error) {
			...
			policy["write_mode_meaning"] = meanings[a.Settings.WriteMode]
			writes := config.AllWriteScopeSet()
			corps := config.CorpScopeSet()
			if len(tokens) == 0 {
				return merge(map[string]any{
					...
					"corporation_scopes_requested": a.Settings.CorpScopes,
				}, policy), nil
			}
	...
	addTool(s, &mcp.Tool{
		...
		return Call(ctx, func(a *app.App) (any, error) {
			...
			scopes := a.Settings.RequestedScopes()
			var writes []string
			if a.Settings.WriteMode != "off" {
				writes = a.Settings.AllowedNames()
				sort.Strings(writes)
			}
