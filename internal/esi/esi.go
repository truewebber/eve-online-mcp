...
func New(settings *config.Settings, httpClient *http.Client, store *cache.Store, sso *auth.Client) *Client {
	return &Client{
		settings:    settings,
		http:        httpClient,
		store:       store,
...
func (c *Client) cacheKey(path string, characterID *int, params map[string]any) string {
	...
	canonical, _ := json.Marshal(map[string]any{"p": path, "c": cid, "q": normalise(params), "d": c.settings.CompatDate})
...
func (c *Client) headers(characterID *int) (http.Header, error) {
	...
	h.Set("X-Compatibility-Date", c.settings.CompatDate)
