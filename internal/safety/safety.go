...
func New(settings *config.Settings) *Guard {
	return &Guard{settings: settings, pending: map[string]pendingWrite{}}
}

func (g *Guard) CheckCapability(capability string) error {
	cap, ok := config.WriteCapabilities[capability]
	if !ok {
		return Blocked{Msg: fmt.Sprintf("Unknown write capability %q.", capability)}
	}
	if g.settings.WriteMode == "off" {
		return Blocked{Msg: "Writes are disabled on this server (write_mode=off). Set write_mode=confirm in the config and restart."}
	}
	if !g.settings.CapabilityEnabled(capability) {
		enabled := g.settings.AllowedNames()
...
func (g *Guard) CheckScope(capability string, granted []string) error {
	cap := config.WriteCapabilities[capability]
...
func (g *Guard) checkBudget(capability string) error {
	...
	if len(g.recentWrites) >= g.settings.WriteBudgetPerHour {
		return Blocked{Msg: fmt.Sprintf("Write budget exhausted: %d writes in the last hour. This is a safety cap, not an ESI limit — wait, or raise write_budget_per_hour.", g.settings.WriteBudgetPerHour)}
	}
	if capability == "mail_send" {
		g.recentMail = trim(g.recentMail, now, time.Hour)
		if len(g.recentMail) >= g.settings.MailBudgetPerHour {
			return Blocked{Msg: fmt.Sprintf("Mail budget exhausted: %d mails in the last hour.", g.settings.MailBudgetPerHour)}
		}
	}
	return nil
}
...
func (g *Guard) Authorize(tool, capability string, args map[string]any, preview map[string]any, confirmToken string, granted []string) (map[string]any, error) {
	...
	return map[string]any{
		...
		"expires_in_seconds": g.settings.ConfirmTTLSeconds,
...
func (g *Guard) auditLocked(entry map[string]any) {
	...
	f, err := os.OpenFile(g.settings.AuditFile(), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600)
	if err != nil {
...
func (g *Guard) expirePending() {
	cutoff := time.Now().Add(-time.Duration(g.settings.ConfirmTTLSeconds) * time.Second)
...
func (g *Guard) Status() map[string]any {
	...
	sort.Strings(enabled)
	disabled := []string{}
	for name := range config.WriteCapabilities {
		if _, ok := g.settings.WriteAllow[name]; !ok {
			disabled = append(disabled, name)
		}
	}
	sort.Strings(disabled)
	ref := map[string]string{}
	for name, cap := range config.WriteCapabilities {
		ref[name] = cap.Summary
	}
	return map[string]any{
		"write_mode": g.settings.WriteMode,
		"enabled_capabilities": enabled,
		"disabled_capabilities": disabled,
		"capability_reference": ref,
		"writes_last_hour": len(g.recentWrites),
		"write_budget_per_hour": g.settings.WriteBudgetPerHour,
		"mails_last_hour": len(g.recentMail),
		"mail_budget_per_hour": g.settings.MailBudgetPerHour,
		"pending_confirmations": len(g.pending),
		"audit_log": g.settings.AuditFile(),
	}
}
